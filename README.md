...
매출총이익률은 매출액 - 매출원가로 매출총이익을 산출한 뒤 매출액으로
나누어 계산합니다. 직접 공시된 매출총이익 계정에 의존하지 않아 기업별
표시 방식 차이에 따른 영향을 줄였습니다.

기업 시계열 분석에서는 현재 정의된 재무비율과 DB에 저장된 ratio_code를
비교합니다. 새 재무비율이 추가되어 과거 연도에 해당 계산 결과가 없으면
이를 자동으로 감지하여 재계산할 수 있습니다. 반면 계산 결과가 NULL인
...
신규 재무비율 추가 후 과거 DB에 해당 ratio_code가 없는 연도 자동
식별
